...
`src/app/section-disclosure.ts` owns native-details registration, per-signature in-memory toggle state, empty reconciliation, focus-safe close, section jump navigation, and visible-render callbacks. Feature controllers provide stable local IDs, labels, availability, defaults, and a state signature. Disclosure state is never serialized. `applyFocusTarget()` opens a containing `details` before focusing and scrolling an existing deep-link target.
