...
export function TrophyPage({ view, owners, onOwnerChange, active, availableSections }: TrophyPageProps) {
  ...
  return <>
    ...
    {available('trophyHardwareDisclosure') && <Disclosure id="trophyHardwareDisclosure" title="Hardware Shelf" open>
      <div id="trophyHardwareShelf" class="trophy-shelf">{view.hardwareShelf.map(item => <article class={`trophy-hardware-card ${item.tone} ${item.state}`} data-hardware-state={item.state} key={item.label} aria-label={`${item.label}: ${item.count} ${item.state === 'earned' ? 'earned' : 'earned so far'}`}><div class="trophy-card-top">{item.icon && <img class="trophy-card-art" src={hardwareArt(item.icon)} alt="" aria-hidden="true" />}<div class="trophy-card-title"><h3 class="trophy-year-chip">{item.label}</h3><span class="trophy-card-state">{item.state === 'earned' ? 'Earned' : 'Still chasing'}</span></div><div class="trophy-card-rank">{Number.isFinite(item.rank) ? `#${item.rank}` : '—'}</div></div><div class="trophy-card-value" aria-label={`${item.count} ${item.label}`}>{item.count}</div><div class="trophy-card-years"><span>Years</span> {item.years.length ? item.years.join(', ') : '—'}</div><p class="trophy-card-context">{item.context}</p></article>)}</div>
    </Disclosure>}
