...
- If the Pages source preflight reports `build_type=legacy`, stop the run and have a repository owner change the source to GitHub Actions. Re-run the current `main` tip after the API reports `build_type=workflow`; do not restore branch publishing or copy an old artifact.
- If `verify_pages` reports a byte mismatch, inspect its expected/observed hashes and whether the run was marked superseded. Propagation retries are bounded; a newer `main` SHA is reported as superseded so it does not accuse the newer release. For a same-SHA failure, inspect the retained screenshot and request/error summary before repairing the verifier or reverting through a new tested tip.
