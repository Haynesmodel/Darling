...
- `npm run typecheck` runs the permissive TypeScript migration gate.
...
- `npm run test:ci` runs the local unit, production build, and UI checks that mirror CI.
...
- `public/assets/` is generated by `scripts/sync_public_assets.cjs` before Vite dev/build so the source `assets/` workflow does not need to change.
