...
function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    phase = 'derived'; writeStatus(args['status-report'], { phase, season: Number(args.season), runtime }); if (process.env.DARLING_CANDIDATE_FAIL_PHASE === phase) throw new Error(`Injected failure at ${phase} phase`); const h2hChanged = prepareDerived(candidateRoot, sourceRoot, args.python, Number(args.season));
