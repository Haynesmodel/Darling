...
export function TrophyPage({ view, owners, onOwnerChange, active, availableSections }: TrophyPageProps) {
  ...
  return <>
    ...
    {available('trophyHardwareDisclosure') && <Disclosure id="trophyHardwareDisclosure" title="Hardware Shelf" open>
      <div id="trophyHardwareShelf" class="trophy-shelf">{view.hardwareShelf.map(item => {
        const loreTrigger = item.state === 'earned'
          ? item.label === 'Darlings' ? 'trophy-championship' : item.label === 'Bagels' ? 'trophy-bagel' : item.label === 'Saunders titles' ? 'trophy-saunders' : null
          : null;
        return <article class={`trophy-hardware-card ${item.tone} state-${item.state}`} data-state={item.state} key={item.label}><div class="trophy-card-top">{item.icon && <img class="trophy-card-art" src={hardwareArt(item.icon)} alt="" aria-hidden="true" />}<div class="trophy-card-title"><div class="trophy-year-chip">{item.label}</div><span class="trophy-card-state">{hardwareStateLabel(item)}</span></div><div class="trophy-card-rank">{Number.isFinite(item.rank) ? `#${item.rank}` : '—'}</div></div><div class="trophy-card-measure"><span class="trophy-card-measure-label">Count</span><strong class="trophy-card-value">{item.count}</strong></div><div class="trophy-card-years"><span class="trophy-card-meta-label">Years</span>{item.years.length ? item.years.join(', ') : '—'}</div><p class="trophy-card-context">{item.context}</p>{loreTrigger && <button type="button" class="btn trophy-lore-trigger" data-lore-trigger={loreTrigger} data-lore-owner={view.owner} data-lore-value={`${view.owner}:${item.label}`} aria-label={`Reveal ${item.label} lore`}>Inspect lore</button>}</article>;
      })}</div>
    </Disclosure>}
