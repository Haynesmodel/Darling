...
function runGate(iso, { dismissed = false, search = '' } = {}) {
  ...
  const store = new Map(dismissed ? [['darling.iu-weekend-welcome.dismissed.2026', 'true']] : []);
...
test('the shell gate covers Hoosier weekend in New York', () => {
  assert.equal(runGate('2026-09-13T03:59:59Z').welcome.hidden, true, 'before the welcome window in New York');
  assert.equal(runGate('2026-09-13T04:00:00Z').welcome.hidden, false, 'welcome window start');
  assert.equal(runGate('2026-09-22T03:59:59Z').welcome.hidden, false, 'Monday night end');
  assert.equal(runGate('2026-09-22T04:00:00Z').welcome.hidden, true, 'after the welcome window');
  assert.equal(runGate('2026-09-19T12:00:00Z', { search: '?tab=draft' }).welcome.hidden, true, 'deep-linked feature route');
  assert.equal(runGate('2026-09-19T12:00:00Z', { search: '?tab=pulse' }).welcome.hidden, false, 'explicit home route');
});

test('dismissal persists and returns focus to the app', () => {
  const result = runGate('2026-09-19T12:00:00Z');
  result.dismiss.click();
  assert.equal(result.welcome.hidden, true);
  assert.equal(result.mainContent.focused, true);
  assert.equal(result.store.get('darling.iu-weekend-welcome.dismissed.2026'), 'true');
  assert.equal(runGate('2026-09-19T12:00:00Z', { dismissed: true }).welcome.hidden, true);
});
