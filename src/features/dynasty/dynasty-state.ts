...
export function resolveDynastyInitialState(input: { seasonSummaries: readonly SeasonSummaryRow[]; urlState?: { dynastyMode?: string | null; dynastyOwner?: string | null; dynastyStart?: number | null; dynastyEnd?: number | null; dynastyMinSeasons?: number | null; dynastySaunders?: boolean | null }; mode?: DynastyMode; owner?: string | null; startSeason?: number | null; endSeason?: number | null; minSeasons?: number; includeSaundersPenalty?: boolean }): DynastyState {
  const seasons = availableDynastySeasons(input.seasonSummaries); const owners = availableDynastyOwners(input.seasonSummaries); const url = input.urlState || {}; const requestedMode = (url.dynastyMode || input.mode || 'all-time') as DynastyMode; const selectedOwner = requestedMode === 'calculator' ? ((url.dynastyOwner || input.owner || owners[0]) && owners.includes(url.dynastyOwner || input.owner || '') ? (url.dynastyOwner || input.owner || owners[0]) : owners[0] || ALL_DYNASTY_TEAMS) : ALL_DYNASTY_TEAMS; const range = normalizeDynastyRange({ availableSeasons: seasons, startSeason: input.startSeason, endSeason: input.endSeason, requestedStartSeason: url.dynastyStart ?? input.startSeason, requestedEndSeason: url.dynastyEnd ?? input.endSeason }); const fallback = normalizeDynastyRange({ availableSeasons: seasons }); return { mode: requestedMode, owner: selectedOwner || ALL_DYNASTY_TEAMS, startSeason: range.startSeason ?? fallback.startSeason, endSeason: range.endSeason ?? fallback.endSeason, requestedStartSeason: range.requestedStartSeason ?? fallback.requestedStartSeason, requestedEndSeason: range.requestedEndSeason ?? fallback.requestedEndSeason, minSeasons: Math.max(1, Number(url.dynastyMinSeasons ?? input.minSeasons ?? 2)), includeSaundersPenalty: url.dynastySaunders ?? input.includeSaundersPenalty ?? true, chartHiddenOwners: [], selectedWindowKey: null, selectedWindowKind: null };
}
